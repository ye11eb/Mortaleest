...
export const deliveryInfo  = createAsyncThunk('auth/deliveryInfo', async ({firstName, secondName, number, adress1, adress2, country, city, state, zipcode}) => {
  try{
    const { data } = await axios.post('auth/deliveryInfo', {
      firstName, secondName, number, adress1, adress2, country, city, state, zipcode
    })
...
export const changeEmail  = createAsyncThunk('auth/changeEmail', async ({email, password}) => {
  try{
